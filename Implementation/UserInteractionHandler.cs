...
namespace Terraria.Plugins.CoderCow.HouseRegions
{
	public class UserInteractionHandler : UserInteractionHandlerBase, IDisposable
	{
		...
		private bool TryExecuteSubCommand(string commandNameLC, CommandArgs args)
		{
			switch (commandNameLC)
			{
				...
				case "info":
					...
					return true;
				case "data":
					this.HouseDataCommand_Exec(args);
					return true;
				case "scan":
		...
		#region [Command Handling /house data]
		private void HouseDataCommand_Exec(CommandArgs args)
		{
			if (args == null || this.IsDisposed)
				return;
			...
			if (args.Parameters.Count != 3)
			{
				...
				{
					this.HouseDataCommand_HelpCallback(args);
					return;
				}
				args.Player.SendErrorMessage("Proper syntax: /house data <Owner> <Index>");
				args.Player.SendInfoMessage("Type /house data help to get more information about this command.");
				return;
			}
			...
			foreach (Region region in TShock.Regions.Regions)
			{
				if (region.Name.Equals(regionName))
				{
					...
					if(region.AllowedGroups.Count > 0)
						lines.Add($"Shared with {region.AllowedGroups.Count} groups");

					PaginationTools.SendPage(
						args.Player, 1, lines, new PaginationTools.Settings
						{
							HeaderFormat = string.Format("Data From This House:"),
						}
					);
					return;
				}
		...
		{
			return this.TryGetAccessibleHouseRegionAtPlayer(player, out _, out region);
		}
